...
export const Comment = ({
  ...
  const [newComment] = useState(renameKeys(comment, newKeys))
  ...
  const [loves, setLoves] = useState(commentLove)
  const [lovecount, setLovecount] = useState(upVoteCount)
  useEffect(() => {
  ...
  useEffect(() => {
    for (var i = 0; i < commentList.length; i++) {
      if (
        commentList[i]?.commentReply?.commentText === newComment?.commentText
      ) {
        setMar(true)
      }
    }
  ...
  return newComment.commentText ? (
    <div
      ...
      style={{
        paddingLeft:
          commentLists[0]?.commentText !== commentText ? "25px" : "0px",
        paddingBottom: "10px",
        position: "relative",
        width: "100%"
      }}
    >
      ...
      >
        ...
        >
          ...
          <br />
          <VoteModal isOpen={isOpen} _id={_id} setIsOpen={setIsOpen} />
          <IonRow>
            <IonCol
        ...
        {more &&
          ...
          commentLists?.data?.commentList?.comments.map((reply, i) => {
            return (
              <Comment
                HandleReply={HandleReply}
                HandleDelete={HandleDelete}
                ...
                id={reply?._id}
                i={i}
                commentList={commentLists?.data?.commentList?.comments}
              />
            )
          })}
      </div>
