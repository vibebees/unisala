...
export const FolderScholarship = ({item, allProps}) => {
  const {
    level = "",
    act = {min: "", max: ""},
    awards = [],
    ...
    {showDetails = false, detailsObj = {
      title: "Requirements",
      data: [
        {
          label: "SAT Score",
          value: sat.min === 0 && sat.max === 0 ? "N/A" : `${sat.min} - ${sat.max}`
        },
        {
          label: "ACT Score",
          value: act.min === 0 && act.max === 0 ? "N/A" : `${act.min} - ${act.max}`
        },
        {
          label: "Level",
          value: level
  ...
  const ModalData = (
    <div>
      <IonList>
        ...
        <DetailItem label="Non Score Eligibility Requirements" value={nonScoreElegibilityReq} />
        <DetailItem label="Scholarship URL" value={
          <a href={scholarshipUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500">
            {scholarshipUrl}
          </a>
        } />
      </IonList>
      ...
      <IonCol className="h-80 bg-neutral-50 px-0 shadow-md rounded-md flex justify-center flex-col items-center min-w-[250px] !shrink-0">
        ...
        <div className="w-full">
          ...
          <IonCol className="items-start flex w-full flex-col gap-1">
            {showDetails && <IonText className="font-medium text-neutral-600">
              {detailsObj.title}
            </IonText>}
            {showDetails && detailsObj?.data?.map((item, index) => (
              <IonText key={index} className="text-center !text-xs capitalize">
                {item.label} : <span className="font-medium text-neutral-700">{item.value}</span>
              </IonText>
            ))}
          </IonCol>
        </div>
